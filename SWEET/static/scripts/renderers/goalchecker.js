export async function goalCheckerRenderer(section) {
    ...
    if (goals.current.length == 0 && goals.complete.length == 0) {
        ...
        holder.appendChild(await this.render({ type: "menu", content: [{ type: "menu-item", title: `Go to the ${schema.displayName} section`, link: `#home/healthy-living/${section.goaltype == "eating"? "healthy-eating": "being-active"}`}]}))
        return holder;
    } else {
