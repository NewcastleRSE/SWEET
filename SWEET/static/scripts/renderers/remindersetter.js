export function reminderRenderer(section) {
    ...
    .then(reminders => {
        ...
        holder.innerHTML = `
            <form>
            <p><input type="checkbox" name="daily" id="daily" ${reminders.daily.reminder? "checked": ""}><label class="h4" for="daily">I would like a daily reminder to take my hormone therapy</label><br>
            Send my reminder by <select name="daily-method" ${reminders.daily.reminder? `value="${reminders.daily.method}"`: "disabled"}><option value="email">Email</option><option value="sms">Text Message</option></select> to <input name="daily-to" type="text" ${reminders.daily.reminder? `value="${reminders.daily.to}"`: "disabled"} />.</p>
            <p><input type="checkbox" name="monthly" id="monthly" ${ reminders.monthly.reminder? "checked": ""}><label class="h4" for="monthly">I would like a regular reminder to collect my prescription</label><br>
            Remind me every <select name="monthly-freq" ${reminders.monthly.reminder? `value="${reminders.monthly.frequency}"`: "disabled"}><option value="one">Month</option><option value="three">Three Months</option></select> starting on <input name="monthly-start" type="date" ${reminders.monthly.reminder? `value="${reminders.monthly.start}"`: "disabled"} />.<br>
            ...
            <input type="submit" value="Update Reminders" disabled>
            </form>
        `
