export function reminderRenderer(section) {
    ...
    .then(reminders => {
        ...
        holder.innerHTML = `
            ...
            Send my reminder by <select name="daily-method"${reminders.daily.reminder? "": " disabled"}><option value="email"${reminders.daily.reminder && reminders.daily.method=="email"? " selected":""}>Email</option><option value="sms"${reminders.daily.reminder && reminders.daily.method=="sms"? " selected":""}>Text Message</option></select> to <input name="daily-to" type="text" minlength="3" maxlength="50" required ${reminders.daily.reminder? `value="${reminders.daily.to}"`: "disabled"} />.</p>
            </fieldset>
            ...
            Send my reminder by <select name="monthly-method"${reminders.monthly.reminder? "": " disabled"}><option value="email"${reminders.monthly.reminder && reminders.monthly.method=="email"? " selected":""}>Email</option><option value="sms"${reminders.monthly.reminder && reminders.monthly.method=="sms"? " selected":""}>Text Message</option></select> to <input name="monthly-to" type="text" minlength="3" maxlength="50" required ${reminders.monthly.reminder? `value="${reminders.monthly.to}"`: "disabled"} /></p>
            </fieldset>
